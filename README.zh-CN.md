...
- `NormalizedSQLiteMemoryRepository` 增加第一版 Phase 2 normalized `memory_records`、`memory_evidence`、`memory_audit_events` 表、索引、CRUD、tombstone delete、operation application 和旧 `MemoryStore` 迁移路径。
...
这还不是完整 Phase 2。LLM extraction、event state persistence、hybrid retrieval、更多 event skills、memory settings APIs 和 privacy governance 会作为后续里程碑继续实现。
